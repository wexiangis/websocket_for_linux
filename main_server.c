...
// epoll最大同时管理控制符数量
#define EPOLL_RESPOND_NUM 10000

//服务器收包缓冲区大小
#define SERVER_PKG_MAX 102400
...
{
    int ret;
    WsData_Type dataType;

    memset(wss->buff, 0, sizeof(wss->buff));
    ret = ws_recv(fd, wss->buff, sizeof(wss->buff), &dataType);

    //这可能是一包客户端请求
    if (ret < 0 &&
        strncmp(wss->buff, "GET", 3) == 0 &&
        strstr(wss->buff, "Sec-WebSocket-Key"))
    {
        //检查该客户端是否未接入过
        if (arrayFind(wss->clientArray, EPOLL_RESPOND_NUM, fd) < 0)
        {
            //构建回复
            ret = ws_responseClient(fd, wss->buff, ret);
            //连接成功后,再添加fd到数组
            if (ret > 0)
    ...
    //回调
        ...
        wss->server_callBack(wss, fd, wss->buff, ret, dataType);
...
void server_thread(void *argv)
{
    ...
    while (!wss->exit)
    {
        ...
        for (count = 0; count < nfds; count++)
        {
            ...
            {
                printf("server: accept close fd/%d\r\n", events[count].data.fd);
            ...
            else if (events[count].events & EPOLLIN)
            {
                ...
                {
                    printf("server: check error fd/%d %d\r\n", events[count].data.fd, errno);
                    ev.data.fd = events[count].data.fd;
                    if (epoll_ctl(epoll_fd, EPOLL_CTL_DEL, events[count].data.fd, &ev) < 0)
                    {
                        printf("server: EPOLL_CTL_DEL failed !\r\n");
...
void server_callBack(Ws_Server *wss, int fd, char *buff, int buffLen, WsData_Type type)
{
    ...
    {
        printf("server: recv fd/%d bad pkg -- %s\r\n", fd, buff);
    }
}
    ...
    while (!wss.exit)
    {
        ...
        for (i = 0; i < EPOLL_RESPOND_NUM; i++)
        {
            if (wss.clientArray[i][1] != 0 && wss.clientArray[i][0] > 0)
            {
                ...
                if (ws_send(client_fd, tips, strlen(tips), false, WDT_TXTDATA) < 0)
                {
                    printf("server: send fd/%d error\r\n", client_fd);
                    //标记损坏
